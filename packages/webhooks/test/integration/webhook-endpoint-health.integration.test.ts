...
describe.skipIf(!hasDatabase)("webhook endpoint health integration", () => {
  ...
  beforeEach(() => {
    setIntegrationFetchHandler(async () => new Response("not found", { status: 404 }));
  });
  ...
  it("auto-disables after client errors and resets via resetWebhookEndpointHealth", async () => {
    ...
    for (let attempt = 0; attempt < ENDPOINT_CLIENT_ERROR_STRIKE_LIMIT; attempt += 1) {
      await processWebhookDeliveries({ deliveryIds: [deliveryId] });
      vi.advanceTimersByTime(CLIENT_ERROR_RETRY_DELAY_MS + 1_000);
    }

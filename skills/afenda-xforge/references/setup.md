...
```bash
git clone https://github.com/pohlai88/afenda-xforgeV2.git
cd afenda-xforgeV2
pnpm install
pnpm env:sync   # optional: pull Vercel env after linking
pnpm env:doctor
```

Copy `.env.example` files to their working equivalents (or use `pnpm env:sync`).
...
Fork the BaseHub CMS template for this project, generate a Read Token, set `BASEHUB_TOKEN`.
